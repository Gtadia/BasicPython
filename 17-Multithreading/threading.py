...
def func4():
    """
    How to run multiple threads easily (instead of creating them manually).
    You can run multiple threads easily by using a loop.
    """
    print("_____")
    import threading
    import time
...
def func7():
    ...
    A future object basically encapsulates the execution of our function and allows us to check in on it after it's been scheduled so we can check that it's running or if 
    it's done, or the result. So, if we grabbed a result, then it'll give us the return value of the function.

    """

    with concurrent.futures.ThreadPoolExecutor() as executor: # context manager
        f1 = executor.submit(do_something, 1) # Takes in a function and an argument for the parameters
...
def func9():
    ...
    In short, you use this function to get the results of the threads (basically like the 'result' function) 
    """
...
def func11():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        seconds = [5, 4, 3, 2, 1]
        results = executor.map(do_something, seconds) # When we used the 'sumbit' method, it returned a future object. When we use 'map', it returns the results.
...
def func13part1():
    ...
    img_urls = [ # The links are commented out because I don't want to download these pictures twice.
...
def func13part2():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        executor.map(download_image, img_urls) # This will go through all the elements in the 'img_urls' list and then create new threads to run these elements through the 'download_image' function.
